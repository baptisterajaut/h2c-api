...
def generate_sa(sa_dir, namespace, extra_hosts=None):
    ...
    subject = x509.Name([x509.NameAttribute(NameOID.COMMON_NAME, "h2c-api")])
...
def build_h2c_api_service(sa_dir, compose_path, runtime_socket, host_port=None):
    """Return the h2c-api compose service dict."""
    h2c_volumes = [
    ...
    svc = {
        ...
        "command": [
            ...
            f" && wget -qO /tmp/h2c_api.py {H2C_API_URL}"
            f" && python3 /tmp/h2c_api.py",
        ],
        "volumes": h2c_volumes,
    }
    if host_port:
...
# ---------------------------------------------------------------------------
# Transform class (h2c extension mode)
# ---------------------------------------------------------------------------

class H2cApiInject:  # pylint: disable=too-few-public-methods  # contract: one class, one method
    ...
    def transform(self, compose_services, ingress_entries, ctx):  # pylint: disable=unused-argument  # Transform contract signature
        """Add h2c-api service and inject SA mount + env into every service."""
        cfg = ctx.config.get("h2c-api") or {}
        ...
        # Compose path — h2c writes compose.yml in output_dir
        compose_path = Path(ctx.output_dir) / ctx.config.get("compose_file", "compose.yml")

        # Add h2c-api service
        compose_services["h2c-api"] = build_h2c_api_service(
            Path(SA_DIR), compose_path, runtime_socket, host_port)
        _log(f"added h2c-api service (socket: {runtime_socket or 'not found'})")
...
def main():
    ...
    override_services["h2c-api"] = build_h2c_api_service(
